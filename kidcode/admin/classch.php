<?php 
  ...
  if(isset($_REQUEST['add_id'])){
     ...
     while($row = mysqli_fetch_array($result_tech, MYSQLI_ASSOC)){
      ...
      $sel = "SELECT * FROM class_schedule WHERE class_schedule_teacher = '$id'";
          ...
          <div class="col-md-12">
            <div class="card">
              ...
              <select name="stu[]" input type="text" class="form-control col-12 col-sm-12" id="provi" name="provi" >
                 <?php if($row_num == 0){
                          $sel_student = "SELECT * FROM student WHERE student_branch = '$branch' AND student_status = 'เรียนอยู่'";
                          $result_student = mysqli_query($conn, $sel_student);
                        ...
                        if($row_num != 0){
                          ...
                          $check = "SELECT * FROM class_schedule WHERE class_schedule_branch = '$branch' AND class_schedule_teacher =  '$id'";
                          $result_check = mysqli_query($conn, $check);
                          while($row = mysqli_fetch_array($result_check, MYSQLI_ASSOC)){
                            $idstu = $row['class_schedule_student'];
                            array_push($no, $idstu);
                          }
                            $realstu = "SELECT * FROM student WHERE student_branch = '$branch'AND student_status = 'เรียนอยู่' AND student_id NOT IN (".implode(',',$no).")";
                            $result_realstu = mysqli_query($conn, $realstu);
                            while($row2 = mysqli_fetch_array($result_realstu, MYSQLI_ASSOC)){?>
                              <option value="<?php echo $row2['student_id'];?>"><?php echo $row2['student_fullname']; ?></option>
                          <?php  }
