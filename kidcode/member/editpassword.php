<?php
    ...
    if(isset($_POST['edit_pass'])){
        ...
        $sql = ("SELECT student_password FROM student WHERE student_id = '$id'");
        $query = mysqli_query($conn, $sql);
        $result = mysqli_fetch_assoc($query);
        $pass3 = md5($pass3);

        if ($result) {      
            if ($result['student_password'] != $pass3) {
        ...
        if (count($errors) == 0) {
            $pass = md5($pass1);
            $sql = "UPDATE student set student_password = '$pass' WHERE student_id = '$id'";
