        ...
        for($i=0;$i<count($_FILES["port"]["name"]);$i++)
                {
                    ...
                    if($_FILES["port"]["name"][$i] != "")
                    {
                        if(move_uploaded_file($_FILES["port"]["tmp_name"][$i], $port))
                        {
                                ...
                                $insert_port = "INSERT INTO portfolio (portfolio_files, portfolio_date, portfolio_student, portfolio_comment, portfolio_subject) values ('$port','$datenow','$stu_id','$text','$subject_id')";
