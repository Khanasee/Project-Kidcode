<?php
  ...
  $major_id = $_SESSION['tech_branch'];

  $select_major = "SELECT * FROM branch WHERE branch_id = '$major_id'";
  $result_major_sel = mysqli_query($conn, $select_major);
 while($row = mysqli_fetch_array($result_major_sel, MYSQLI_ASSOC)) {
   $major_name = $row['branch_name'];
 }
 $sel_tech = "SELECT * FROM tech WHERE major = '$major_id'";
 $result_tech = mysqli_query($conn, $sel_tech);
  ...
  $result_member = mysqli_query($conn, $member);
   ...
   $show = ("SELECT *  FROM teacher_schedule AS d1
            ...
            ON d1.teacher_schedule_subject=d4.subject_id where teacher_schedule_branch = '$major_id' AND member_id = '$id'");
    ...
    <div class="modal-content bg-default">
      ...
      <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control col-sm-12" id="idmajor" name="major" value="<?php echo $major_name;?>">
        ...
        <select class="custom-select form-control-border" id="exampleSelectBorder" name="tech[]" >
          <?php while($row= mysqli_fetch_array($result_tech, MYSQLI_ASSOC)){
              $tech = $row['tech'];
              $sql = "SELECT * FROM member WHERE member_id = '$tech'";
              $resulttech = mysqli_query($conn, $sql);
              while($row2 = mysqli_fetch_array($resulttech, MYSQLI_ASSOC)){?>
              <option><?php echo $row2['member_fullname'];?></option>
              <?php }}?>
              </select>
          </div>
          <div class="form-group">
